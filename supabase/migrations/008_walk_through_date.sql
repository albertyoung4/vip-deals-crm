-- Track the scheduled walk-through date on a deal.

set search_path to crm, public;

alter table deal add column if not exists walk_through_date date;

-- Rebuild v_deal_dashboard to expose walk_through_date (keeps all prior fields
-- from 006, just adds walk_through_date in the base CTE).
